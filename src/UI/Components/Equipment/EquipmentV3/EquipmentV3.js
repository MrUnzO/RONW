...
define(function (require) {
	 ...
	 *
	 * return {boolean} false to sto the browser from folloin thk
	 */
	function showTab() {
		...
		this.ui
			...
			.html(
				'<div class="item" data-index="' +
					...
					'<span class="itemName">' +
					add3Dots(jQuery.escape(DB.getItemName(item, { showItemGrade: false, showItemSlots: false, showItemOptions: false })), 19) +
					'</span>' +
					'</div>',
			);
	...
	var renderCharacter = (function renderCharacterClosure() {
		...
		var HasCartState = StatusConst.EffectState.CART1 | StatusConst.EffectState.CART2 | StatusConst.EffectState.CART3 | StatusConst.EffectState.CART4 | StatusConst.EffectState.CART5;
	...
	function onDragOver(event) {
		if (window._OBJ_DRAG_) {
			...
			if (data.type === 'item') {
				...
				if ((item.type === ItemType.WEAPON || item.type === ItemType.ARMOR || item.type === ItemType.SHADOWGEAR) && item.IsIdentified && !item.IsDamaged) {
	...
	function onDrop(event) {
		...
		if (data && data.type === 'item') {
			...
			if ((item.type === ItemType.WEAPON || item.type === ItemType.ARMOR || item.type === ItemType.AMMO || item.type === ItemType.SHADOWGEAR) && item.IsIdentified && !item.IsDamaged) {
				EquipmentV3.ui.find('td').css('backgroundImage', 'none');
