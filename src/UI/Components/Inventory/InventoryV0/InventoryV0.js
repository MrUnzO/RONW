...
define(function (require) {
	...
	InventoryV0.addItemSub = function AddItemSub(item) {
		...
		if (tab === _preferences.tab) {
			...
			var itemObj = jQuery(
				'<div class="item-container"><div class="item-inventory item" data-index="' +
					item.index +
					'" draggable="true">' +
					'<div class="icon"></div>' +
		...
		if (item.count > 1) {
			...
			InputBox.onSubmitRequest = function OnSubmitRequest(count) {
				...
				switch (data.from) {
					...
					case 'WriteRodex':
						getModule('UI/Components/Rodex/WriteRodex').requestRemoveItemRodex(item.index, parseInt(count, 10));
						break;
				}
			};
			return false;
	...
	function onItemOver() {
		...
		let quantity = ' ea';
		if (item.Options && (item.type === ItemType.WEAPON || item.type === ItemType.ARMOR || item.type === ItemType.SHADOWGEAR) && item.Options.filter(Option => Option.index !== 0).length > 0) {
			quantity = ' Quantity';
		}
