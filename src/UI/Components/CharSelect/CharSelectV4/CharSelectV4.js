...
define(function (require) {
	...
	function formatDuration(seconds) {
		...
		let formattedDuration = DB.getMessage(3349).replace('%d', `${hours}`).replace('%d', `${minutes}`).replace('%d', `${remainingSeconds}`);
		...
		if (UIready) {
			...
			if (backgroundchange && shouldRunBackgroundChange === true) {
				Client.loadFile(DB.INTERFACE_PATH + 'select_character_ver3/img_slot_select' + img + '.bmp', function (dataURI) {
					backgroundchange.css({ backgroundImage: 'url(' + dataURI + ')', width: '157px', height: '197px', 'background-size': 'contain', 'background-repeat': 'no-repeat' });
				});
