		...
		if (hover) {
			Client.loadFile(DB.INTERFACE_PATH + hover, function (dataURI) {
				...
				$node.mouseover(function () {
					this.style.backgroundImage = 'url(' + hover_uri + ')';
				});
				$node.mouseout(function () {
					this.style.backgroundImage = bg_uri ? 'url(' + bg_uri + ')' : '';
				});
			});
		}
		...
		if (down) {
			Client.loadFile(DB.INTERFACE_PATH + down, function (dataURI) {
				...
				$node.mouseup(function () {
					this.style.backgroundImage = 'url(' + (hover_uri || bg_uri) + ')';
				});
			});
