...
define(function (require) {
	...
	Damage.init = function init(gl) {
		...
		Client.getFiles(
			['data/sprite/\xc0\xcc\xc6\xd1\xc6\xae/\xbc\xfd\xc0\xda.spr', 'data/sprite/\xc0\xcc\xc6\xd1\xc6\xae/msg.spr', 'data/sprite/\xc0\xcc\xc6\xd1\xc6\xae/bluemsg.spr'],
	...
	Damage.add = function add(damage, entity, tick, weapon, type) {
		...
		for (i = 0, count = numbers.length; i < count; ++i) {
			frame = _numbers[numbers[i]];
			ctx.drawImage(frame, start_x + width, start_y + ((height - frame.height) >> 1));
			width += frame.width + PADDING;
		}
